...
L'établissement de ce catalogue découle d'une analyse exploratoire approfondie menée sur un échantillon réel de 5 000 dossiers de personnes physiques issus des tables du périmètre (\textbf{\underline{environ 15 tables \texttt{CCM\_*}, dont \texttt{ccm\_npdescription} prise ici à titre d'exemple illustratif}}). Cette analyse a révélé des anomalies typiques et récurrentes de saisie au sein du système opérant Oracle NOVA, ce qui a directement guidé le choix et le paramétrage de nos dimensions de contrôle :
\begin{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=stealth, every node/.style={font=\small}, box/.style={draw=blue!80!black, rectangle, fill=blue!5, rounded corners, minimum width=2.8cm, minimum height=1cm, align=center, drop shadow}]
        ...
        \node (dag_dq) [box, fill=orange!10, draw=orange!60!black, below=of dag_ing, yshift=-1.0cm] {DAG \texttt{daily\_dq\_00} \\ \scriptsize(Qualité à 00h00)};
