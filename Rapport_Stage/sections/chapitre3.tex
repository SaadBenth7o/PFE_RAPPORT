...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{
    \begin{tikzpicture}[node distance=2.4cm, >=stealth, every node/.style={font=\normalsize}]
        % Lifelines
        ...
        \node (hdfs) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2.4cm, minimum height=1cm, drop shadow, right=of sqoop] {\textbf{Hive / HDFS}};
        \node (soda) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2.6cm, minimum height=1cm, drop shadow, right=of hdfs, align=center] {\textbf{Data Quality}\\\textbf{Engine}};
        ...
        % Execution arrows
        \draw [->, line width=1.2pt] ($(airflow)+(0,-1.5)$) -- node[above, font=\small, text width=2.5cm, align=center] {1. Planifier\\l'ingestion} ($(sqoop)+(0,-1.5)$);
        
        \draw [->, line width=1.2pt] ($(sqoop)+(0,-2.6)$) -- node[above, font=\small, text width=3.5cm, align=center] {2. Importer données\\(depuis NOVA)} ($(hdfs)+(0,-2.6)$);
        
        \draw [<-, dashed, line width=1.2pt, color=green!50!black] ($(sqoop)+(0,-3.5)$) -- node[above, font=\small, text width=3cm, align=center] {3. OK (Succès)} ($(hdfs)+(0,-3.5)$);
        
        \draw [->, line width=1.2pt] ($(airflow)+(0,-4.5)$) -- node[above, font=\small, text width=4.5cm, align=center] {4. Lancer les scans\\de Data Quality} ($(soda)+(0,-4.5)$);
        
        \draw [->, line width=1.2pt] ($(soda)+(0,-5.4)$) -- node[above, font=\small, text width=3cm, align=center] {5. Évaluer règles\\sur la zone RAW} ($(hdfs)+(0,-5.4)$);
        
        \draw [<-, dashed, line width=1.2pt, color=gray!80!black] ($(soda)+(0,-6.3)$) -- node[above, font=\small, text width=3cm, align=center] {6. Données brutes} ($(hdfs)+(0,-6.3)$);
        
        \draw [->, line width=1.2pt] ($(soda)+(0,-7.2)$) -- node[above, font=\small, text width=3cm, align=center] {7. Sauvegarder\\résultats (3 tables)} ($(hdfs)+(0,-7.2)$);
        
        \draw [->, line width=1.2pt] ($(tableau)+(0,-8.2)$) -- node[above, font=\small, text width=3cm, align=center] {8. Requêter le\\Dashboard} ($(dremio)+(0,-8.2)$);
        
        % Arrow from Dremio to HDFS (crosses Soda)
        \draw [->, line width=1.2pt] ($(dremio)+(0,-9.0)$) -- node[above, font=\small, text width=3cm, align=center, pos=0.8] {9. Interroger les\\3 tables physiques} ($(hdfs)+(0,-9.0)$);
        
        % Arrow from HDFS to Dremio (crosses Soda)
        \draw [<-, dashed, line width=1.2pt, color=gray!80!black] ($(dremio)+(0,-9.8)$) -- node[above, font=\small, text width=3cm, align=center, pos=0.8] {10. Retour des\\données} ($(hdfs)+(0,-9.8)$);
        
        \draw [<-, dashed, line width=1.2pt, color=blue!60!black] ($(tableau)+(0,-10.5)$) -- node[above, font=\small, text width=3.2cm, align=center] {11. Vues sémantiques\\consolidées} ($(dremio)+(0,-10.5)$);
