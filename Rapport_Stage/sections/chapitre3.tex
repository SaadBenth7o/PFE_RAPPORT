...
\begin{longtable}{|p{0.18\textwidth}|p{0.18\textwidth}|p{0.22\textwidth}|p{0.12\textwidth}|p{0.23\textwidth}|}
\caption{Catalogue des règles DQ — Dimension Complétude} \label{tab:regles_completude} \\
\hline
\rowcolor{gray!20}
\textbf{Périmètre} & \textbf{Colonne(s)} & \textbf{Description Règle / Seuil} & \textbf{Criticité} & \textbf{Action corrective} \\
\hline
\endfirsthead
\hline
\rowcolor{gray!20}
...
\hline
\endhead
KYC (\texttt{ccm\_npdescription}) & \texttt{name} & 0 valeur nulle tolérée & CRITIQUE & Audit dossiers - Blocage saisie agence \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{firstname} & 0 valeur nulle tolérée & CRITIQUE & Audit dossiers - Blocage saisie agence \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{birthdate} & 0 valeur nulle tolérée & CRITIQUE & Saisie obligatoire avec justificatif officiel \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{nationality\_}\allowbreak\texttt{country} & 0 valeur nulle tolérée & HAUTE & Renseigner le code pays de nationalité \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{professional\_}\allowbreak\texttt{sector} & Taux de valeurs nulles < 5\% & HAUTE & Renseigner le secteur d'activité de l'employeur \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{occupation} & Taux de valeurs nulles < 5\% & HAUTE & Préciser la profession exacte du client \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{socioprofessional\_}\allowbreak\texttt{category} & Taux de valeurs nulles < 5\% & HAUTE & Renseigner la catégorie socioprofessionnelle (CSP) \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{professional\_}\allowbreak\texttt{situation} & Taux de valeurs nulles < 5\% & HAUTE & Renseigner le statut de la situation professionnelle \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{employer} & Taux de valeurs nulles < 10\% & HAUTE & Renseigner le nom de l'entreprise employeuse \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{politically\_}\allowbreak\texttt{exposed} & 0 valeur nulle tolérée (PEP obligatoire) & CRITIQUE & Renseigner la déclaration d'exposition politique \\
\hline
\end{longtable}

\subsubsection{Dimension Validité}
...
\begin{longtable}{|p{0.18\textwidth}|p{0.18\textwidth}|p{0.22\textwidth}|p{0.12\textwidth}|p{0.23\textwidth}|}
\caption{Catalogue des règles DQ — Dimension Validité} \label{tab:regles_validite} \\
\hline
\rowcolor{gray!20}
\textbf{Périmètre} & \textbf{Colonne(s)} & \textbf{Description Règle / Seuil} & \textbf{Criticité} & \textbf{Action corrective} \\
\hline
...
\hline
\endhead
KYC (\texttt{ccm\_npdescription}) & \texttt{name}, \texttt{firstname} & Longueur $\ge$ 2, aucun chiffre ou caractère spécial & HAUTE & Nettoyage et normalisation dans NOVA \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{birthdate} & Taux de dates génériques au 01/01 < 15\% & HAUTE & Recherche de la date réelle sur pièce d'identité \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{civility} & Valeurs restreintes à (M., MME, MLLE) & HAUTE & Restauration de la valeur selon pièces fournies \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{sex} & Valeurs restreintes à (M, F) & HAUTE & Restauration de la valeur selon pièces fournies \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{birth\_city} & Doit exister dans le référentiel des villes BAM & MOYENNE & Mapping et correction via la table de transcodage \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{professional\_}\allowbreak\texttt{sector} & Code numérique valide dans le référentiel sectoriel & MOYENNE & Transcodage vers les codes officiels BAM \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{professional\_}\allowbreak\texttt{situation} & Valeurs restreintes à (SAL, FON, TRI, AUT, EMP, NDE) & HAUTE & Remplacer par un code référentiel valide \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{employer} & Détection des valeurs parasites ('AUTRE', 'LUI MEME') & HAUTE & Normaliser : mapper 'LUI MEME' vers auto-entrepreneur \\
\hline
\end{longtable}
...
\begin{longtable}{|p{0.18\textwidth}|p{0.18\textwidth}|p{0.22\textwidth}|p{0.12\textwidth}|p{0.23\textwidth}|}
\caption{Catalogue des règles DQ — Dimension Cohérence} \label{tab:regles_coherence} \\
\hline
\rowcolor{gray!20}
\textbf{Périmètre} & \textbf{Colonne(s)} & \textbf{Description Règle / Seuil} & \textbf{Criticité} & \textbf{Action corrective} \\
\hline
...
\hline
\endhead
KYC (\texttt{ccm\_npdescription}) & \texttt{sex} + \texttt{civility} & Genre M $\neq$ MME/MLLE ; Genre F $\neq$ M. & CRITIQUE & Résolution manuelle des incohérences de saisie \\
\hline
KYC (\texttt{ccm\_npdescription}) & \texttt{fatca\_}\allowbreak\texttt{identification\_}\allowbreak\texttt{number} & Si nationalité ou double nationalité = US, 0 valeur nulle & CRITIQUE & Déclaration réglementaire FATCA immédiate \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{employer} + \texttt{professional\_}\allowbreak\texttt{situation} & Si situation = NDE (sans emploi) ou TRI (retraité), employeur doit être NULL & HAUTE & Annuler le nom d'employeur incohérent dans NOVA \\
\hline
Transverse & \texttt{code\_segment} & Cohérence entre segment client et code\_marche & HAUTE & Recalculer le segment par défaut \\
\hline
\end{longtable}
...
\begin{longtable}{|p{0.18\textwidth}|p{0.18\textwidth}|p{0.22\textwidth}|p{0.12\textwidth}|p{0.23\textwidth}|}
\caption{Catalogue des règles DQ — Dimension Unicité} \label{tab:regles_unicite} \\
\hline
\rowcolor{gray!20}
\textbf{Périmètre} & \textbf{Colonne(s)} & \textbf{Description Règle / Seuil} & \textbf{Criticité} & \textbf{Action corrective} \\
\hline
...
\hline
\endhead
KYC (\texttt{ccm\_npdescription}) & \texttt{oid} / \texttt{radical} & Unicité stricte du numéro de radical client (0 doublon) & CRITIQUE & Fusion des doublons clients dans le référentiel \\
\hline
CSP (\texttt{ccm\_job}) & \texttt{ordnum} & Un seul enregistrement actif principal (\texttt{main=1}) par tiers & HAUTE & Éliminer les contrats de travail multiples actifs \\
\hline
\end{longtable}

\subsubsection{Dimension Fraîcheur et Pipeline}
...
\begin{longtable}{|p{0.18\textwidth}|p{0.18\textwidth}|p{0.22\textwidth}|p{0.12\textwidth}|p{0.23\textwidth}|}
\caption{Catalogue des règles DQ — Dimension Fraîcheur et Pipeline} \label{tab:regles_fraicheur} \\
\hline
\rowcolor{gray!20}
\textbf{Périmètre} & \textbf{Colonne(s)} & \textbf{Description Règle / Seuil} & \textbf{Criticité} & \textbf{Action corrective} \\
\hline
...
\hline
\endhead
Pipeline & \texttt{batch\_}\allowbreak\texttt{datetime} & Retard d'ingestion HDFS RAW < 26h & HAUTE & Alerte infrastructure Big Data et relance Sqoop \\
\hline
Pipeline & \texttt{row\_count} & Écart de volumétrie entre Oracle NOVA et Hive RAW < 0.1\% & CRITIQUE & Rejouer le DAG d'ingestion Sqoop \\
\hline
\end{longtable}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \node at (6,2) [align=center, text=gray] {\textbf{Capture d'écran à insérer} \\ \small Emplacement de la vue sémantique dans Dremio};
    \end{tikzpicture}
    \caption{Emplacement de la vue virtuelle unique \texttt{v\_dq\_tiers\_results\_fact} dans Dremio}
