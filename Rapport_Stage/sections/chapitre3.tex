...
Le diagramme de classes de la figure \ref{fig:class_dq} représente la structure des données générée par notre moteur de qualité. Contrairement à une table de faits unique, le moteur Soda Core produit un ensemble de trois tables physiques liées par l'identifiant d'exécution (\texttt{run\_id}). Ces trois tables sont ensuite unifiées au sein de Dremio pour exposer une table de faits consolidée complète pour le reporting.

\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{
    \begin{tikzpicture}[every node/.style={font=\scriptsize}, classbox/.style={draw, rectangle split, rectangle split parts=2, fill=blue!5, rounded corners, align=left}]
        
        \node (colhist) [classbox, text width=5.5cm] {
            \textbf{consolidated\_all\_column\_history}
            \nodepart{second}
            + run\_id : String \textbf{[PK]}\\
            + colonne\_controlee : String \\
            + chemin\_source : String \\
            + table\_dataset : String \\
            + query\_id : String \textbf{[PK]}\\
            + domaine\_metier : String \\
            + type\_regle\_dq : String \\
            + volume\_total : Long \\
            + lignes\_conformes : Long \\
            + score\_qualite\_pct : Double \\
            + statut : String (PASS/WARN/FAIL)\\
            + date\_execution : Timestamp\\
            + criticite : String
        };

        \node (checks) [classbox, text width=6cm, right=of colhist, xshift=0.5cm, yshift=2cm] {
            \textbf{consolidated\_all\_checks\_config}
            \nodepart{second}
            + run\_id : String \textbf{[FK]}\\
            + run\_date : Date \\
            + query\_id : String \textbf{[FK]}\\
            + table : String \\
            + domain : String \\
            + rule : String \\
            + virt\_col : String \\
            + virt\_full\_path : String \\
            + raw\_col : String \\
            + sql : String \\
        ...
        \node (exec) [classbox, text width=6cm, right=of colhist, xshift=0.5cm, yshift=-2cm] {
            \textbf{consolidated\_execution\_log}
            \nodepart{second}
            + run\_id : String \textbf{[FK]}\\
            + run\_date : Date \\
            + query\_id : String \textbf{[FK]}\\
            ...
            + virt\_full\_path : String \\
            + raw\_col : String \\
            + check\_execution\_status : String \\
            + check\_error\_message : String \\
            + check\_timestamp : Timestamp \\
            + run\_execution\_status : String \\
            + run\_error\_message : String \\
            + updated\_at : Timestamp
        };

        \node (dremio) [draw, rectangle split, rectangle split parts=2, fill=green!5, rounded corners, align=left, text width=6cm, right=of checks, xshift=0.5cm, yshift=-2cm] {
            \textbf{fact\_tiers\_dq\_consolidated (Vue Dremio)}
            \nodepart{second}
            \textit{Jointure des 3 tables (21 colonnes)} \\
            - Regroupe l'historique, la configuration \\
            et le statut d'exécution pour Tableau.
        };

        \draw[->, dashed] (checks.west) -- node[above, sloped] {lié par run\_id, query\_id} (colhist.east);
        \draw[->, dashed] (exec.west) -- node[below, sloped] {lié par run\_id, query\_id} (colhist.east);
        
        \draw[->, thick, green!50!black] (colhist.east) to[bend left=10] (dremio.west);
        \draw[->, thick, green!50!black] (checks.east) to[bend left=10] (dremio.west);
        \draw[->, thick, green!50!black] (exec.east) to[bend left=10] (dremio.west);
        
    \end{tikzpicture}
    }
    \caption{Diagramme de classes des tables physiques et de la vue consolidée}
    \label{fig:class_dq}
\end{figure}
...
\begin{itemize}
    \item \textbf{consolidated\_all\_column\_history} ([\underline{run\_id}, \underline{query\_id}], colonne\_controlee, chemin\_source, table\_dataset, domaine\_metier, type\_regle\_dq, volume\_total, lignes\_conformes, score\_qualite\_pct, statut, date\_execution, criticite).
    \item \textbf{consolidated\_all\_checks\_config} ([\underline{run\_id}$^{\#}$, \underline{query\_id}$^{\#}$], run\_date, table, domain, rule, virt\_col, virt\_full\_path, raw\_col, sql, updated\_at, criticite).
    \item \textbf{consolidated\_execution\_log} ([\underline{run\_id}$^{\#}$, \underline{query\_id}$^{\#}$], run\_date, table, domain, rule, virt\_col, virt\_full\_path, raw\_col, check\_execution\_status, check\_error\_message, check\_timestamp, run\_execution\_status, run\_error\_message, updated\_at).
\end{itemize}
