...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=stealth, every node/.style={font=\small}, box/.style={draw=blue!80!black, rectangle, fill=blue!5, rounded corners, minimum width=3.5cm, minimum height=1.2cm, align=center, drop shadow}]
        ...
        \node (ingest) [box, right=2cm of dag_ing] {Module d'Ingestion \\ \scriptsize(Apache Sqoop)};

        % Ligne 2
        \node (lake) [box, below=1cm of ingest] {Zone de Stockage RAW \\ \scriptsize(Hive / HDFS)};

        % Ligne 3
        \node (soda) [box, below=1cm of lake] {Moteur DQ \\ \scriptsize(Soda Core / Python)};
        \node (dag_dq) [box, fill=orange!10, draw=orange!60!black, left=2cm of soda] {DAG \texttt{daily\allowbreak\_dq\allowbreak\_00} \\ \scriptsize(Qualité à 00h00)};

        % Ligne 4
        \node (res) [box, below=1cm of soda] {Résultats DQ \\ \scriptsize(Hive / HDFS)};

        % Ligne 5
        \node (dremio) [box, below=1cm of res] {Couche de Virtualisation \\ \scriptsize(Dremio)};

        % Ligne 6
        \node (tableau) [box, below=1cm of dremio] {Dashboard \\ \scriptsize(Tableau)};

        % Connectors
        \draw [->, line width=1pt, dashed, orange!80!black] (dag_ing) -- node[above, font=\scriptsize] {Déclenche} (ingest);
        \draw [->, line width=1pt, blue!70!black] (ingest) -- node[right, font=\scriptsize] {Écrit} (lake);
        
        \draw [->, line width=1pt, dashed, orange!80!black] (dag_dq) -- node[above, font=\scriptsize] {Déclenche} (soda);
        
        \draw [->, line width=1pt, blue!70!black] (soda) -- node[right, font=\scriptsize] {Scan} (lake);
