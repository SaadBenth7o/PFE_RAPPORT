...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node (scan_pipe) [block] at (0,-3.3) {\textbf{Contrôles Pipeline} \\ (Fraîcheur, Volumétrie)};
        \node (dec_pipe) [decision] at (0,-5.5) {\textbf{Pipeline} \\ \textbf{Sain ?}};
        \node (alert_pipe) [block, fill=red!10, draw=red!60!black] at (5,-5.5) {\textbf{Alerte Bloquante} \\ Arrêt du DAG};
        \node (end_fail) [draw, circle, double=black, fill=black, inner sep=3pt] at (5,-7.5) {};
        
        \node (scan_biz) [block] at (0,-7.8) {\textbf{Exécution Règles Métier} \\ (Validité, Complétude...)};
        \node (dec_biz) [decision] at (0,-10.0) {\textbf{Anomalies} \\ \textbf{détectées ?}};
        
        \node (score_ok) [block, fill=green!10, draw=green!60!black] at (5,-10.0) {\textbf{Validation} \\ Score = 100\%};
        \node (end_ok) [draw, circle, double=black, fill=black, inner sep=3pt] at (5,-12.0) {};
        
        \node (score_fail) [block, fill=orange!10, draw=orange!60!black] at (0,-12.0) {\textbf{Historisation Anomalie}};
        \node (remed) [block, fill=purple!10, draw=purple!60!black] at (0,-14.0) {\textbf{Correction Manuelle} \\ (Action Data Steward)};
        
        % Arrows
        \draw [->, thick] (start) -- (ingest);
        \draw [->, thick] (ingest) -- (scan_pipe);
        \draw [->, thick] (scan_pipe) -- (dec_pipe);
        \draw [->, thick, color=red!60!black] (dec_pipe) -- node[above, font=\small, font=\bfseries] {Non} (alert_pipe);
        \draw [->, thick] (alert_pipe) -- (end_fail);
        
        \draw [->, thick, color=green!60!black] (dec_pipe) -- node[right, font=\small, font=\bfseries] {Oui} (scan_biz);
        \draw [->, thick] (scan_biz) -- (dec_biz);
        
        \draw [->, thick, color=green!60!black] (dec_biz) -- node[above, font=\small, font=\bfseries] {Non (0 erreur)} (score_ok);
        \draw [->, thick] (score_ok) -- (end_ok);
        
        \draw [->, thick, color=red!60!black] (dec_biz) -- node[right, font=\small, font=\bfseries] {Oui (> 0)} (score_fail);
        \draw [->, thick] (score_fail) -- (remed);
        
        % Loop back
        \draw [->, thick, dashed] (remed) .. controls (-4.5,-14.0) and (-4.5,-1.5) .. node[left, font=\small, align=center] {Nouveau passage\\(J+1)} (ingest);
    \end{tikzpicture}
    }
...
\begin{itemize}
    \item \textbf{Le phénomène des dates génériques} : Une concentration anormale de dates de naissance fixées au premier janvier a été identifiée, traduisant une saisie par défaut dans le système lorsque la date exacte n'est pas connue. Cela justifie notre règle de validité temporelle avec un seuil de tolérance métier adapté ,
    \item \textbf{Le codage des champs de référence} : Des champs textuels (comme les villes) contiennent parfois des codes numériques issus d'anciens systèmes, nécessitant un contrôle strict par rapport au référentiel central pour assurer la cohérence des données ,
    \item \textbf{La cohérence conditionnelle réglementaire (ex. FATCA)} : Certains identifiants fiscaux internationaux doivent obligatoirement répondre à des règles de remplissage croisées très strictes, dépendantes d'autres champs (comme le pays de nationalité ou de résidence fiscale).
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=triangle 60, every node/.style={font=\small}, 
        comp/.style={draw=blue!80!black, rectangle, fill=blue!5, rounded corners=1mm, minimum width=4cm, minimum height=1.4cm, align=center, drop shadow},
        db/.style={draw=blue!80!black, cylinder, shape border rotate=90, aspect=0.25, fill=blue!5, minimum width=4cm, minimum height=1.6cm, align=center, drop shadow},
        orchestrator/.style={draw=orange!60!black, rectangle, fill=orange!10, rounded corners=1mm, minimum width=4cm, minimum height=1.4cm, align=center, drop shadow}]
        
        % Ligne 1
        \node (dag_ing) [orchestrator] {$\ll$ordonnanceur$\gg$ \\ \textbf{DAG \texttt{dailyow22}} \\ \scriptsize(Ingestion à 22h00)};
        \node (ingest) [comp, right=2cm of dag_ing] {$\ll$composant$\gg$ \\ \textbf{Module d'Ingestion} \\ \scriptsize(Apache Sqoop)};

        % Ligne 2
        \node (lake) [db, below=1cm of ingest] {$\ll$base de données$\gg$ \\ \textbf{Zone RAW} \\ \scriptsize(Hive / HDFS)};

        % Ligne 3
        \node (soda) [comp, below=1cm of lake] {$\ll$composant$\gg$ \\ \textbf{Moteur DQ} \\ \scriptsize(Soda Core / Python)};
        \node (dag_dq) [orchestrator, left=2cm of soda] {$\ll$ordonnanceur$\gg$ \\ \textbf{DAG \texttt{daily\_dq\_00}} \\ \scriptsize(Qualité à 00h00)};

        % Ligne 4
        \node (res) [db, below=1cm of soda] {$\ll$base de données$\gg$ \\ \textbf{Résultats DQ} \\ \scriptsize(Hive / HDFS)};

        % Ligne 5
        \node (dremio) [comp, below=1cm of res] {$\ll$composant$\gg$ \\ \textbf{Couche Virtualisation} \\ \scriptsize(Dremio)};

        % Ligne 6
        \node (tableau) [comp, below=1cm of dremio] {$\ll$composant$\gg$ \\ \textbf{Dashboard} \\ \scriptsize(Tableau)};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        >=stealth,
        font=\small,
        server/.style={draw=blue!80!black, rectangle, fill=blue!5, rounded corners, minimum width=4.5cm, minimum height=2.5cm, align=center, drop shadow, thick},
        cluster/.style={draw=orange!80!black, rectangle, rounded corners, inner sep=15pt, fill=orange!5, dashed, thick},
        ext/.style={draw=gray!80!black, cylinder, shape border rotate=90, aspect=0.25, fill=gray!10, minimum width=3.5cm, minimum height=2cm, align=center, drop shadow, thick},
        bi/.style={draw=purple!80!black, rectangle, fill=purple!5, rounded corners, minimum width=3.5cm, minimum height=2cm, align=center, drop shadow, thick}
    ]

        % Coordinates for alignment
        \node (edge) [server] at (0, 0) {\textbf{1 Nœud Edge (Gateway)} \\[0.1cm] \scriptsize $\bullet$ Apache Airflow (Workers) \\ \scriptsize $\bullet$ Soda Core (Python venv) \\ \scriptsize $\bullet$ Dremio Coordinator \\ \scriptsize $\bullet$ Hive Metastore};

        \node (masters) [server] at (6.5, 0) {\textbf{3 Nœuds Masters} \\[0.1cm] \scriptsize $\bullet$ HDFS NameNodes (HA) \\ \scriptsize $\bullet$ YARN ResourceManager \\ \scriptsize $\bullet$ JournalNodes};

        \node (workers) [server] at (6.5, -5) {\textbf{3 Nœuds Workers} \\[0.1cm] \scriptsize $\bullet$ HDFS DataNodes (RAW) \\ \scriptsize $\bullet$ YARN NodeManagers \\ \scriptsize $\bullet$ Dremio Executors};

        % Cluster background
        \node (cluster_bg) [cluster, fit=(edge) (masters) (workers), label={[font=\bfseries, text=orange!80!black]above:Cluster Big Data Cloudera CDP (Production)}] {};

        % External Systems
        \node (oracle) [ext] at (-5.5, 0) {\textbf{Oracle NOVA} \\[0.1cm] \scriptsize Système Opérant \\ \scriptsize (Source de données)};

        \node (tableau) [bi] at (12.5, -5) {\textbf{Serveur Tableau} \\[0.1cm] \scriptsize Visualisation BI \\ \scriptsize (Dashboard DQ)};

        % Connections
        \draw [->, thick, color=blue!70!black] (oracle) -- node[above, font=\scriptsize, align=center] {Ingestion Sqoop} (edge);
        
        \draw [->, thick, color=blue!70!black] (edge) -- node[above, font=\scriptsize, align=center] {Ordonnancement\\\& Métadonnées} (masters);
        
        \draw [<->, thick, color=blue!70!black] (masters) -- node[right, font=\scriptsize, align=left] {Ressources\\(YARN)} (workers);
        
        \draw [->, thick, color=blue!70!black] (edge) |- node[above, font=\scriptsize, near end, xshift=-1cm] {Exécution distribuée (Spark / Dremio)} (workers);
        
        \draw [->, thick, color=blue!70!black] (workers) -- node[above, font=\scriptsize, align=center] {Flux ODBC} (tableau);
...
\begin{minipage}[c]{0.22\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth, height=1.0cm, keepaspectratio]{figures/logos/dbeaver.png}
...
\begin{minipage}[c]{0.73\textwidth}
    ...
    {\small Client SQL universel utilisé durant la phase de cadrage. Suite à l'ouverture de flux par l'équipe réseau, DBeaver nous a permis d'accéder au système opérant en lecture seule afin d'explorer la structure des tables sources et d'analyser un échantillon de données pour définir nos règles de qualité.}
\end{minipage}
...
\begin{minipage}[c]{0.22\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth, height=1.0cm, keepaspectratio]{figures/logos/logo_sqoop.png}
    \captionsetup{font=scriptsize}\renewcommand{\figurename}{Fig.}
    \captionof{figure}{Apache Sqoop}
\end{minipage}
\hspace{0.03\textwidth}
\begin{minipage}[c]{0.73\textwidth}
...
\begin{minipage}[c]{0.22\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth, height=1.0cm, keepaspectratio]{figures/logos/logo_soda.png}
    \captionsetup{font=scriptsize}\renewcommand{\figurename}{Fig.}
...
\begin{minipage}[c]{0.22\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth, height=1.0cm, keepaspectratio]{figures/logos/logo_airflow.png}
    \captionsetup{font=scriptsize}\renewcommand{\figurename}{Fig.}
...
\begin{minipage}[c]{0.22\textwidth}
    \centering
    \includegraphics[width=0.8\textwidth, height=1.5cm, keepaspectratio]{figures/logos/logo_gitlab.png}
    \captionsetup{font=scriptsize}\renewcommand{\figurename}{Fig.}
