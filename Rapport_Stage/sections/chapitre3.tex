...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{
    \begin{tikzpicture}[node distance=3.0cm, >=stealth, every node/.style={font=\scriptsize}]
        % Lifelines
        \node (airflow) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2cm, minimum height=1cm, drop shadow] {\textbf{Airflow}};
        \node (sqoop) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2cm, minimum height=1cm, drop shadow, right=of airflow] {\textbf{Sqoop}};
        \node (hdfs) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2cm, minimum height=1cm, drop shadow, right=of sqoop] {\textbf{Hive / HDFS}};
        \node (soda) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2.4cm, minimum height=1cm, drop shadow, right=of hdfs, align=center] {\textbf{Data Quality}\\\textbf{Engine}};
        \node (dremio) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2cm, minimum height=1cm, drop shadow, right=of soda] {\textbf{Dremio}};
        \node (tableau) [draw=blue!80!black, fill=blue!10, rounded corners, minimum width=2cm, minimum height=1cm, drop shadow, right=of dremio] {\textbf{Tableau}};

        % Dotted lines
        \draw [dashed, draw=gray, thick] (airflow) -- +(0,-10.5);
        \draw [dashed, draw=gray, thick] (sqoop) -- +(0,-10.5);
        \draw [dashed, draw=gray, thick] (hdfs) -- +(0,-10.5);
        \draw [dashed, draw=gray, thick] (soda) -- +(0,-10.5);
        \draw [dashed, draw=gray, thick] (dremio) -- +(0,-10.5);
        \draw [dashed, draw=gray, thick] (tableau) -- +(0,-10.5);

        % Execution arrows
        \draw [->, line width=1.2pt] ($(airflow)+(0,-1.5)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {1. Planifier l'ingestion} ($(sqoop)+(0,-1.5)$);
        
        \draw [->, line width=1.2pt] ($(sqoop)+(0,-2.6)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {2. Importer données} ($(hdfs)+(0,-2.6)$);
        
        \draw [<-, dashed, line width=1.2pt, color=green!50!black] ($(sqoop)+(0,-3.5)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {3. OK (Succès)} ($(hdfs)+(0,-3.5)$);
        
        \draw [->, line width=1.2pt] ($(airflow)+(0,-4.5)$) -- node[above, font=\scriptsize, text width=5cm, align=center] {4. Lancer les scans de Data Quality} ($(soda)+(0,-4.5)$);
        
        \draw [->, line width=1.2pt] ($(soda)+(0,-5.4)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {5. Évaluer règles sur RAW} ($(hdfs)+(0,-5.4)$);
        
        \draw [<-, dashed, line width=1.2pt, color=gray!80!black] ($(soda)+(0,-6.3)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {6. Données brutes} ($(hdfs)+(0,-6.3)$);
        
        \draw [->, line width=1.2pt] ($(soda)+(0,-7.2)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {7. Sauvegarder (3 tables)} ($(hdfs)+(0,-7.2)$);
        
        \draw [->, line width=1.2pt] ($(tableau)+(0,-8.2)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {8. Requêter le Dashboard} ($(dremio)+(0,-8.2)$);
        
        % Arrow from Dremio to HDFS (crosses Soda)
        \draw [->, line width=1.2pt] ($(dremio)+(0,-9.0)$) -- node[above, font=\scriptsize, text width=5cm, align=center, pos=0.7] {9. Interroger les 3 tables} ($(hdfs)+(0,-9.0)$);
        
        % Arrow from HDFS to Dremio (crosses Soda)
        \draw [<-, dashed, line width=1.2pt, color=gray!80!black] ($(dremio)+(0,-9.8)$) -- node[above, font=\scriptsize, text width=5cm, align=center, pos=0.7] {10. Retour des données} ($(hdfs)+(0,-9.8)$);
        
        \draw [<-, dashed, line width=1.2pt, color=blue!60!black] ($(tableau)+(0,-10.5)$) -- node[above, font=\scriptsize, text width=2.6cm, align=center] {11. Vues consolidées} ($(dremio)+(0,-10.5)$);
