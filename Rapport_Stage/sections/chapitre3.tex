...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node (fact) [factbox] {
            ...
            - Date de run \hfill \textit{String (YYYY-MM-DD)}\\
        ...
        \node (checks) [tablebox, below left=of fact, xshift=-0.5cm, yshift=4cm] {
            ...
            + run\_id \textbf{[PK]} \hfill \textit{String}\\
            + query\_id \textbf{[PK]} \hfill \textit{String}\\
            + run\_date \hfill \textit{String (YYYY-MM-DD)}\\
            + table \hfill \textit{String}\\
...
La dimension de fraîcheur et de flux s'assure du bon cadencement de l'ingestion quotidienne et de l'intégrité de la chaîne technique. Afin de s'adapter à la croissance naturelle du référentiel bancaire, le contrôle de volumétrie (\texttt{row\_count}) n'est jamais figé en dur (\textit{hardcodé}) : il compare dynamiquement le volume extrait au jour $J$ avec la volumétrie validée du jour précédent ($J-1$) ainsi qu'avec le comptage de la source Oracle NOVA. Toute déviation anormale au-delà du seuil de tolérance déclenche une alerte bloquante avant de mettre à jour la nouvelle référence pour le jour suivant.
...
\hline
\texttt{row\_count} & Dérive volumétrique dynamique (Source vs Cible et $J$ vs $J-1$) < 0.1\% & CRITIQUE & Déclencher le Circuit Breaker et rejouer l'ingestion \\
\hline
\end{longtable}
