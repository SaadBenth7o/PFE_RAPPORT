...
La planification, le déclenchement et le monitoring des scans de qualité sont gérés par un \textbf{nouveau DAG Airflow indépendant}, spécifiquement dédié au contrôle DQ. Conformément aux standards de l'équipe Data Office, ce DAG est intégré dans le projet d'orchestration \texttt{errays} et packagé sous forme de fichier Wheel (\texttt{.whl}) via le gestionnaire \textbf{PDM}, avant d'être déployé dans le répertoire \texttt{\$DAGS\_FOLDER} d'Airflow. Afin de découpler totalement l'ingestion de la vérification de la qualité, ce nouveau DAG est planifié pour s'exécuter quotidiennement à \textbf{00h00}. Cela laisse une marge de sécurité de deux heures après le déclenchement du DAG d'ingestion Sqoop principal (\texttt{dailyow22}, prévu à 22h00). L'architecture d'exécution repose sur la soumission du moteur DQ via \texttt{spark-submit} avec le fichier Wheel du projet DQ en argument \texttt{--py-files}, suivant le même pattern de déploiement que les projets de transformation existants (\texttt{py-khawarizmi}, \texttt{sddiqi}). Cette séparation modulaire assure qu'une défaillance dans le DAG d'ingestion ne bloque pas ou ne corrompt pas la supervision de la qualité.
...
\begin{enumerate}
    ...
    \item \textbf{Détail Métier} : Permet une analyse fine par colonne et par dimension de qualité sur le domaine métier Tiers ,
