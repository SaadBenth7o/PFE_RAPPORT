...
\begin{itemize}
    \item \textbf{Sous-besoin 1.1 : Déclenchement automatique} : Le système doit lancer l'évaluation de la qualité des données de manière planifiée, immédiatement après l'exécution complète du batch quotidien d'ingestion des tables sources Oracle NOVA (DAG Airflow \texttt{dailyow22} s'exécutant chaque soir à 22h00) ,
...
\begin{itemize}
    \item \textbf{Sous-besoin 3.1 : Persistance journalière} : À chaque exécution, le système doit sauvegarder les métriques calculées et les résultats de chaque contrôle de qualité dans une table d'historique de scores (base Hive dédiée \texttt{dq\_results}), indexée par date d'exécution ,
...
\hline
Airflow (Orchestrateur) & Système & Déclenche automatiquement le scan DQ chaque soir après l'ingestion (DAG \texttt{dailyow22}). \\
\hline
