...
Une unique vue sémantique virtuelle de la table de faits est créée dans Dremio sous le schéma \texttt{dq-tiers}. Dremio sert de couche d'exposition et de virtualisation directe sans déplacement ni duplication physique des données. 

Le script SQL de création du dataset virtuel est le suivant :

\begin{lstlisting}[language=SQL]
-- Vue Dremio : exposition de la table de faits unique
CREATE OR REPLACE VIEW "dq-tiers"."v_dq_tiers_results_fact" AS
SELECT
    run_date,
    run_timestamp,
    dag_name,
    ...
    state,
    score_dq
FROM hive.dq_results.dq_tiers_results_fact;
\end{lstlisting}

Afin d'optimiser les performances de requêtage, aucune pré-agrégation SQL ou filtre n'est appliqué au niveau de Dremio. L'exposition brute de cette table de faits permet de déléguer toute l'intelligence de calcul et de regroupement à l'outil BI Tableau Desktop.
...
Le dashboard Tableau est connecté à Dremio via le connecteur ODBC/JDBC natif. L'unique vue virtuelle \texttt{"dq-tiers"."v\_dq\_tiers\_results\_fact"} est importée comme source de données unique dans Tableau Desktop. 
