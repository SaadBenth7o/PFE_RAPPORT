...
def run_dq_checks(run_date: str):
    ...
    if results_data:
        df = spark.createDataFrame(results_data, [
            ...
            "invalid_rows", "percentage", "passed", "score_dq", "run_date"
        ])
...
\subsection{Vue \texttt{v.dq\_anomalies\_summary}}

\begin{lstlisting}[language=SQL]
-- Vue Dremio : regles en anomalie (filtree sur passed = false)
CREATE OR REPLACE VIEW "dq-tiers"."v.dq_anomalies_summary" AS
SELECT
    run_date,
    table_name,
    column_name,
...
Le dashboard Tableau est connecté à Dremio via le connecteur ODBC/JDBC Dremio. Les trois vues du schéma \texttt{dq-tiers} sont importées comme sources de données dans Tableau : \texttt{v.dq\_scores\_history}, \texttt{v.dq\_kpis\_summary} et \texttt{v.dq\_anomalies\_summary}.
