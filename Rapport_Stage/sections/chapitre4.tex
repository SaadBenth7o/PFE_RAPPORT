...
Ce chapitre présente la réalisation concrète du pipeline de Data Quality sur le Référentiel Tiers de CIH Bank. Tous les contrôles Soda Core s'effectuent exclusivement sur la \textbf{zone RAW} du Data Lake (base Hive \texttt{raw\_nova\_referentieltiers}), qui constitue la copie exacte et non transformée des tables \texttt{CCM\_*} du système opérant Oracle NOVA. Nous détaillerons successivement l'environnement technique de développement, l'implémentation du script Soda Core, la création des tables Hive de persistance, l'intégration dans un DAG Airflow indépendant, la mise en place des vues Dremio et la réalisation du dashboard Tableau.
...
% =========================
\section{Création du DAG Airflow indépendant \texttt{dq\_tiers\_daily}}

\subsection{Ajout de la tâche DQ}

Un nouveau DAG nommé \texttt{dq\_tiers\_daily} a été créé pour automatiser les contrôles de qualité de façon totalement indépendante de l'ingestion. Placé à 00h00, ce DAG exploite le \texttt{BashOperator} pour déclencher un script d'enveloppe Shell (\texttt{run\_dq.sh}), garantissant une exécution modulaire et isolée du moteur Python Soda Core sur le nœud Edge.

\begin{lstlisting}[language=Python]
# DAG indépendant dq_tiers_daily (Airflow)
...
Ce nouveau DAG \texttt{dq\_tiers\_daily} s'exécute quotidiennement à \textbf{00h00}, deux heures après l'ingestion. La tâche \texttt{BashOperator} lance le script shell qui gère en interne le ticket Kerberos (\texttt{kinit}) puis déclenche le job PySpark. Les logs Airflow permettent de suivre l'exécution et de détecter toute anomalie dans le pipeline DQ.
...
\begin{itemize}
    ...
    \item La date et l'heure de la dernière exécution du batch d'ingestion (\texttt{dailyow22})
\end{itemize}
